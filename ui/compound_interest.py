...
class CompoundInterestWidget(QWidget):
    ...
    def recompute(self) -> None:
        ...
        ymax = float(val_series.max())
        ymin = float(val_series.min())
    ...
    def _on_hover(self, event):
        ...
        if ax is self._value_ax and self._scatter is not None:
            ...
            if contains and ind.get("ind"):
                ...
                for other_ax, other_marker in self._line_markers.items():
                    if other_ax is not self._value_ax:
        ...
        # 2) Tooltip sulla linea più vicina nell'asse corrente
        data = self._axes_data.get(ax)
        if data is None or event.xdata is None:
            annot = self._annot_by_ax.get(ax)
            marker = self._line_markers.get(ax)
